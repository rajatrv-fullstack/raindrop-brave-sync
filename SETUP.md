...
4. **Read the id on the card and confirm it matches the id from step 3.** If it differs,
   `manifest.json` has no `key` (or the wrong one) and the id came from the install path —
   fix the manifest, remove the card, and load again.
...
millisecond; each independently saw the URL as absent and created it. This is not hypothetical —
it produced a real duplicate during testing.
...
- Raindrop's `/user/stats` → `meta.changedBookmarksDate` gate semantics. It is unconfirmed
  ...
  rather than trusting a zero-call exit path. The design also calls for a max-staleness override
  — a forced full read after 7 days — because nothing documents which mutations advance it.
